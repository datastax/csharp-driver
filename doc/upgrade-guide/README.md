...
There's a [dedicated guide](upgrade-from-dse-driver) for DSE driver users that plan to migrate to the `CassandraCSharpDriver`.

## Our policy regarding API changes and release versioning

You can find details about our policy on API changes and release versioning [on this page](api-changes-policy).

## 4.0 - New major release

There's a [dedicated guide](upgrade-to-4x) for driver users that plan to migrate to the new major version of the driver, i.e., version 4.
