    ...
    public class TimeUuidSerializationTests : SingleNodeClusterTest
    {
        ...
        public void SerializationTests()
        {
            ...
            for (var i = 0; i < rowIdArray.Length; i++)
            {
                ...
                Session.Execute(_insertPrepared.Bind(rowId, (Guid)timeUuid));
                var row = Session.Execute(_selectPrepared.Bind(rowId)).FirstOrDefault();
                Assert.NotNull(row);
                Assert.AreEqual(timeUuid, (TimeUuid)row.GetValue<Guid>("timeuuid_sample"));
                Assert.AreEqual(timeUuid.GetDate(), ((TimeUuid)row.GetValue<Guid>("timeuuid_sample")).GetDate());
