...
namespace Cassandra.IntegrationTests.Cloud
{
    ...
    public class CloudIntegrationTests : SharedCloudClusterTest
    {
        ...
        {
            var ex = Assert.Throws<InvalidOperationException>(() => Cassandra.DseCluster.Builder().WithCloudSecureConnectionBundle("does-not-exist.zip").Build());
        }
        ...
        [Test]
        public async Task Dse_Should_UseCorrectConsistencyLevelDefaults_When_Dbaas()
        {
