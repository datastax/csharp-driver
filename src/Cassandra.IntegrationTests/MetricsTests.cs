    ...
    public class MetricsTests : SharedClusterTest
    {
        ...
        public void Should_RemoveNodeMetricsAndDisposeMetricsContext_When_HostIsRemoved()
        {
            ...
            try
            {
                ...
                {
                    Assert.AreEqual(2, cluster.Metadata.Hosts.Count, "metadata hosts count failed");
                }, 200, 50);
                TestHelper.RetryAssert(() =>
                {
                    Assert.AreEqual(2, metrics.NodeMetrics.Count, "Node metrics count failed");
                }, 10, 500);
            }
            catch
            {
            ...
            TestHelper.RetryAssert(() =>
            {
                Assert.AreEqual(3, cluster.Metadata.Hosts.Count, "metadata hosts count after bootstrap failed");
            }, 200, 50);
            ...
            TestHelper.RetryAssert(() =>
            {
                Assert.AreEqual(3, metrics.NodeMetrics.Count, "Node metrics count after bootstrap failed");
            }, 10, 500);
        ...
        public void Should_AllMetricsHaveValidValues()
        {
            ...
            Assert.AreEqual(26,
                MetricsTests.Counters.Concat(MetricsTests.Gauges.Concat(MetricsTests.Timers.Concat(MetricsTests.Meters)))
                            .Union(NodeMetric.AllNodeMetrics).Count());
            ...
            Assert.IsTrue(SessionMetric.AllSessionMetrics.SequenceEqual(SessionMetric.AllSessionMetrics.Distinct()));
