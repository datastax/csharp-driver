        ...
        private string getExceptionAndAllInnerEx(Exception ex, bool recur = false)
        {
            ...
            sb.Append(string.Format("( Exception! Source {0} \n Message: {1} \n StackTrace:\n {2} ", ex.Source, ex.Message,
                                    (Diagnostics.CassandraStackTraceIncluded
                                         ...
                                         : "To display StackTrace, change Debugging.StackTraceIncluded property value to true."), category));
        ...
        public void Error(Exception ex)
        {
            if (ex != null) //shouldn't happen
            {
                if (Diagnostics.CassandraTraceSwitch.TraceError)
                    Trace.WriteLine(
                        string.Format("{0} #ERROR: {1}", DateTimeOffset.Now.DateTime.ToString(DateFormat), getExceptionAndAllInnerEx(ex)), category);
            }
            else
