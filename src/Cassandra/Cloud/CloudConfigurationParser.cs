...
namespace Cassandra.Cloud
{
    ...
    internal class CloudConfigurationParser : ICloudConfigurationParser
    {
        ...
        private void ValidateConfiguration(CloudConfiguration config)
        {
            ...
            {
                CloudConfigurationParser.Logger.Warning("Could not parse the \"username\" property from the config file.");
            }

            if (string.IsNullOrEmpty(config.Password))
            {
                CloudConfigurationParser.Logger.Warning("Could not parse the \"password\" property from the config file.");
            }
