...
namespace Cassandra
{
    internal class OutputRows : IOutput
    {
        ...
        internal OutputRows(FrameReader reader, IRequest request, Guid? traceId)
        {
            ResultRowsMetadata = new RowSetMetadata(reader);
            _rowLength = reader.ReadInt32();
            TraceId = traceId;
            RowSet = new RowSet();
            ProcessRows(RowSet, reader, request);
        }

        /// <summary>
        /// Process rows and sets the paging event handler
        /// </summary>
        internal void ProcessRows(RowSet rs, FrameReader reader, IRequest request)
        {
