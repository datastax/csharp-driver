...
namespace Dse.Test.Integration.SimulacronAPI.PrimeBuilder.Then
{
    public class RowsResult : IRowsResult
    {
        ...
        {
            _columnNamesToTypes = columnNamesToTypes.Select(tuple => (tuple.Item1, tuple.Item2.AdaptForSimulacron().Value)).ToArray();
        }
        ...
        public IRowsResult WithRow(params object[] values)
        {
            ...
            {
                _columnNamesToTypes = _columnNames.Zip(values, (name, val) => (name, DataType.GetDataType(val).AdaptForSimulacron().Value)).ToArray();
            }
            ...
            _rows.Add(_columnNamesToTypes.Zip(values, (val1, val2) => (val1.Item1, DataType.AdaptValueForSimulacronPrime(val2))).ToArray());
            return this;
        }
