...
// ported to C# from https://github.com/dain/snappy/blob/master/src/main/java/org/iq80/snappy/SnappyDecompressor.java
...
namespace Snappy
{
    class SnappyDecompressor
    {
        ...
        private static readonly int MaxIncrementCopyOverflow = 20;
        ...
        public static int Uncompress(byte[] compressed, int compressedOffset, int compressedSize, byte[] uncompressed, int uncompressedOffset)
        {
            ...
            SnappyInternalUtils.CheckArgument(expectedLength <= uncompressed.Length - uncompressedOffset,
                     "Uncompressed length {0} must be less than {1}", expectedLength, uncompressed.Length - uncompressedOffset);
                ...
                int outputOffset)
        {
            ...
            while (ipIndex < ipLimit - 5)
            {
                ...
                int trailerBytes = (int)((uint)entry >> 11);
        ...
        private static int[] DecompressTagSlow(byte[] input, int ipIndex, byte[] output, int outputLimit, int outputOffset, int opIndex)
        {
            ...
            {
                case 4:
                    trailer = (input[ipIndex + 3] & 0xff) << 24; goto case 3;
                case 3:
                    trailer |= (input[ipIndex + 2] & 0xff) << 16; goto case 2;
                case 2:
                    trailer |= (input[ipIndex + 1] & 0xff) << 8; goto case 1;
                case 1:
                    trailer |= (input[ipIndex] & 0xff); break;
            }
        ...
        {
            return SnappyInternalUtils.LoadInt(data, index) & _wordmask[bytes];
        }
        ...
        private static void CopyLiteral(byte[] input, int ipIndex, byte[] output, int opIndex, int length)
        {
            ...
            {
                int fastLength = (int)(((uint)length) & 0xFFFFFFF8);
                if (fastLength <= 64)
                {
                    // copy long-by-long
        ...
        private static readonly int[] _wordmask = new int[]{
            0, 0xff, 0xffff, 0xffffff, -1
    };
        ...
        private static readonly short[] _opLookupTable = new short[]{
